import tilesReducer, { ITilesState } from '../../reducers/tiles';

describe("Tiles reducer spec", () => {
    let state: ITilesState;
    ...
    it("returns all tiles given a grid size", () => {
        // Arrange
        const _intialState: ITilesState = {
            tiles: {},
            memory_tiles: [],
            tiles_flipped: 0
        };
        ...
        // Act
        const grid = tilesReducer(_intialState, {
             type: "NEW_GAME", 
             gridSize: 8,
             lvl: "easy"
        });
    ...
    it("adds alreadyMatchedTiles to the state", () => {
        ...
        // Act
        const expected = true;
        const grid = tilesReducer(state, action);
